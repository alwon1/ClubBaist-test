...
1. **Caller -> Availability API**
   - **Inputs:** requested date or date range, preferred start/end time window, reservation participants, optional player segment (member/guest), optional locale/time zone, correlation/request ID.
   - **Outputs:** normalized query object and request acknowledgment metadata.
2. **Availability API -> SeasonService**
   - **Inputs:** requested date range.
...
4. **Availability API -> AvailabilityService slot-inventory component**
   - **Inputs:** filtered date/time range, participant constraints.
...
5. **Availability API -> BookingPolicyService**
   - **Inputs:** caller segment, participants, timing context, candidate slot set.
   - **Outputs:** per-slot policy eligibility (allowed/disallowed) and policy reason codes.
6. **Availability API -> Caller**
   - **Outputs:** available slot list (only slots passing SeasonService + BookingPolicyService checks), optional excluded-slot reasons, response timestamp for client-side staleness handling.
...
1. **Caller -> Reservation API (Create)**
   - **Inputs:** idempotency key, requester identity/context, desired slot identifier (or time tuple), participants, participant details (as required), optional notes, correlation/request ID.
   - **Outputs:** accepted create command envelope.
2. **Reservation API -> SeasonService**
   - **Inputs:** slot date/time.
...
4. **Reservation API -> BookingPolicyService**
   - **Inputs:** requester context, participants, booking timing, slot context.
...
6. **Reservation API -> AvailabilityService slot-inventory component (capacity check + provisional hold/update intent)**
   - **Inputs:** slot identifier, reservation participants, idempotency key, expected occupancy state (if provided).
   - **Outputs:** capacity decision (fits/conflict), resulting occupancy state or conflict details.
...
8. **Reservation API -> ReservationService persistence component**
   ...
   - **Outputs:** reservation record identifier, status and timestamps.
...
### Purpose
Process reservation changes (modify participants/slot) or cancellation, with correct capacity release/reallocation under concurrent activity.

### Sequence
1. **Caller -> Reservation API (Update or Cancel)**
   - **Inputs:** reservation identifier, operation type (update/cancel), requested new slot and/or new participants (update only), idempotency key, requester context, correlation/request ID.
   - **Outputs:** accepted command envelope.
2. **Reservation API -> ReservationService persistence component**
   - **Inputs:** reservation identifier.
   - **Outputs:** current reservation state, current slot allocation and reservation status from ReservationService.
...
5. **Reservation API -> BookingPolicyService (update only as needed)**
   - **Inputs:** requester context, current vs target slot/time, current vs target participants.
...
7. **Reservation API -> AvailabilityService slot-inventory component (capacity adjustment transaction intent)**
   - **Inputs:**
     - **Cancel:** current slot, current participants, release intent, idempotency key.
     - **Update (same slot, participant change):** delta participants and expected occupancy state.
     - **Update (slot change):** release old slot capacity + acquire target slot capacity with concurrency guards.
   - **Outputs:** adjusted capacity state(s), conflict indicator(s), updated occupancy state(s).
...
9. **Reservation API -> ReservationService persistence component**
   ...
   - **Outputs:** persisted reservation status in ReservationService.
10. **Reservation API -> Caller**
   - **Outputs:** final reservation state (updated/canceled), effective slot and participants, and audit timestamps.
