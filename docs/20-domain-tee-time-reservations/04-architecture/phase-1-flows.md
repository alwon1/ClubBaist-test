...
2. **Availability API -> Season/Calendar Service**
...
3. **Decision point: season invalid**
...
4. **Availability API -> Slot Inventory Service**
...
5. **Availability API -> Reservation Policy Service**
...
6. **Availability API -> Caller**
   - **Outputs:** available slot list (only slots passing season/calendar + policy checks), optional excluded-slot reasons, response timestamp/version token for client-side staleness handling.
...
2. **Reservation API -> Season/Calendar Service**
...
2. **Reservation API -> Reservation Store/Service**
   - **Inputs:** reservation identifier.
   - **Outputs:** current reservation state, current slot allocation, reservation version/status.
3. **Reservation API -> Season/Calendar Service (update only when slot/date changes)**
   - **Inputs:** target slot date/time.
   - **Outputs:** season/calendar validity.
4. **Decision point: season invalid**
   - For invalid target season/date on update, return **update rejected: season invalid**.
5. **Reservation API -> Reservation Policy Service (update only as needed)**
   - **Inputs:** requester context, current vs target slot/time, current vs target party size.
   - **Outputs:** policy pass/fail with diagnostics.
6. **Decision point: policy fail**
   - If failed, return **update rejected: policy fail**.
7. **Reservation API -> Slot Inventory Service (capacity adjustment transaction intent)**
   - **Inputs:**
