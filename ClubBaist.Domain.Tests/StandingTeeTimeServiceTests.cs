...
public class StandingTeeTimeServiceTests
{
    ...
    public async Task SubmitRequestAsync_ValidRequest_Succeeds()
    {
        ...
        var bookingMember = await DomainTestData.CreateMemberAsync(userManager, db, level, "bm@test.com", "Booking", "Member");
        var p1 = await DomainTestData.CreateMemberAsync(userManager, db, level, "p1@test.com", "Player", "One");
        var p2 = await DomainTestData.CreateMemberAsync(userManager, db, level, "p2@test.com", "Player", "Two");
        var p3 = await DomainTestData.CreateMemberAsync(userManager, db, level, "p3@test.com", "Player", "Three");
    ...
    public async Task CancelAsync_WrongMember_ReturnsFalse()
    {
        ...
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var bookingMember = await DomainTestData.CreateMemberAsync(userManager, db, level, "bm@test.com", "Booking", "Member");
        var other = await DomainTestData.CreateMemberAsync(userManager, db, level, "other@test.com", "Other", "Member");
        var p1 = await DomainTestData.CreateMemberAsync(userManager, db, level, "p1@test.com", "Player", "One");
        var p2 = await DomainTestData.CreateMemberAsync(userManager, db, level, "p2@test.com", "Player", "Two");
        var p3 = await DomainTestData.CreateMemberAsync(userManager, db, level, "p3@test.com", "Player", "Three");
