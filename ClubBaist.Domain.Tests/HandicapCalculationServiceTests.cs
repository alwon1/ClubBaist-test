...
public class HandicapCalculationServiceTests
{
    ...
    private static async Task<MemberShipInfo> CreateMemberWithGenderAsync(
        ...
        Gender gender)
    {
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", $"Shareholder-{Guid.NewGuid():N}");
        var member = await DomainTestData.CreateMemberAsync(userManager, db, level, email, "Test", "Member");
        ...
        List<uint?> scores)
    {
        ...
        var slot = await DomainTestData.CreateSlotAtAsync(db, start);
