...
public class MembershipApplicationServiceTests
{
    ...
    public async Task SubmitMembershipApplicationAsync_PersistsApplication_WithSubmittedStatus()
    {
        ...
        var associate = await DomainTestData.CreateMembershipLevelAsync(db, "AS", "Associate");
        var sponsor1 = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "sponsor1@test.com", "Sponsor", "One");
        var sponsor2 = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "sponsor2@test.com", "Sponsor", "Two");

        var application = DomainTestData.CreateApplication(associate, sponsor1.Id, sponsor2.Id, email: "applicant@test.com", firstName: "Jane", lastName: "Doe");
    ...
    public async Task SubmitMembershipApplicationAsync_DuplicateActiveApplication_ReturnsFalse()
    {
        ...
        var first = DomainTestData.CreateApplication(associate, sponsor1.Id, sponsor2.Id, email: "duplicate@test.com");
        var second = DomainTestData.CreateApplication(associate, sponsor1.Id, sponsor2.Id, email: "duplicate@test.com", firstName: "Second");
    ...
    public async Task ApproveMembershipApplicationAsync_CreatesUserAndMembership_AndMarksAccepted()
    {
        ...
        var application = DomainTestData.CreateApplication(associate, sponsor1.Id, sponsor2.Id, email: "approved@test.com", firstName: "Alex", lastName: "Approved");
    ...
    public async Task SetApplicationStatusAsync_NonTerminalStatus_UpdatesApplication()
    {
        ...
        var application = DomainTestData.CreateApplication(associate, sponsor1.Id, sponsor2.Id, email: "onhold@test.com");
        Assert.IsTrue(await applicationService.SubmitMembershipApplicationAsync(application));
...
public class MembershipServiceTests
{
    ...
    public async Task SetMembershipLevelForUserAsync_ExistingMembership_UpdatesLevel()
    {
        ...
        var silver = await DomainTestData.CreateMembershipLevelAsync(db, "SV", "Silver");
        var member = await DomainTestData.CreateMemberAsync(userManager, db, bronze, "member@test.com", "Member", "User");
...
public class MembershipLevelServiceTests
{
    ...
    public async Task UpdateMembershipLevelAsync_WithEntityOverload_UpdatesValues()
    {
        ...
        var original = await DomainTestData.CreateMembershipLevelAsync(db, "JR", "Junior");
    ...
    public async Task CreateMembershipLevelAsync_WhenSaveFails_Rethrows()
    {
        ...
        var db = provider.GetRequiredService<AppDbContext>();
        var service = new MembershipLevelService(new ThrowingAppDbContext(provider.GetRequiredService<DbContextOptions<AppDbContext>>(), throwOnSave: true));
    ...
    public async Task UpdateMembershipLevelAsync_WhenSaveFails_Rethrows()
    {
        ...
        var db = provider.GetRequiredService<AppDbContext>();
        var original = await DomainTestData.CreateMembershipLevelAsync(db, "TMP", "Temp");
        var service = new MembershipLevelService(new ThrowingAppDbContext(provider.GetRequiredService<DbContextOptions<AppDbContext>>(), throwOnSave: true));
    ...
    private sealed class ThrowingAppDbContext(DbContextOptions<AppDbContext> options, bool throwOnSave) : AppDbContext(options)
    {
...
public class BookingServiceTests
{
    ...
    public async Task UpdateBookingAsync_ExistingBooking_ReplacesAdditionalParticipants()
    {
        ...
        var shareholder = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var bookingMember = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "booker@test.com", "Booker", "User");
        var firstParticipant = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "player1@test.com", "Player", "One");
        var secondParticipant = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "player2@test.com", "Player", "Two");
    ...
    public async Task UpdateBookingAsync_WhenParticipantHasNearbyBooking_ReturnsFalse_AndLeavesExistingParticipants()
    {
        ...
        var shareholder = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var bookingMember = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "booker@test.com", "Booker", "User");
        var existingParticipant = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "player1@test.com", "Player", "One");
        var conflictingParticipant = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "player2@test.com", "Player", "Two");
        var secondBookingMember = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "booker2@test.com", "Booker", "Two");

        var (_, firstSlot) = await DomainTestData.CreateSeasonAndSlotAsync(
            seasonService,
            db,
            DateOnly.FromDateTime(DateTime.Today.AddDays(1)),
            new TimeOnly(8, 0));
    ...
    public async Task CreateBooking_OutsideSeason_ReturnsFalse()
    {
        ...
        var shareholder = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var member = await DomainTestData.CreateMemberAsync(userManager, db, shareholder, "booker@test.com", "Booker", "User");
