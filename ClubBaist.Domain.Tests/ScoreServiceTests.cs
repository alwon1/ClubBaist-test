...
public class ScoreServiceTests
{
    ...
    private static async Task<(Season season, TeeTimeSlot slot)> PastSlotAsync(
        SeasonService seasonService, AppDbContext db)
    {
        var date = DateOnly.FromDateTime(DateTime.Today.AddDays(-2));
        return await DomainTestData.CreateSeasonAndSlotAsync(seasonService, db, date, new TimeOnly(8, 0));
    }
    ...
    public async Task T01_GetEligible_NoPastBookings_ReturnsEmpty()
    {
        ...
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var member = await DomainTestData.CreateMemberAsync(userManager, db, level, "m1@t.com", "A", "B");
    ...
    public async Task T02_GetEligible_OnePlayerBooking1HourAgo_ReturnedEmpty_InsideLock()
    {
        ...
        var slot = await DomainTestData.CreateSlotAtAsync(db, slotTime);
        await DomainTestData.CreateBookingAsync(db, member, slot);
    ...
    public async Task T04_GetEligible_AlreadyScoredBooking_ExcludedFromResults()
    {
        ...
        var member = await DomainTestData.CreateMemberAsync(userManager, db, level, "m1@t.com", "A", "B");
        var (_, slot) = await PastSlotAsync(seasonService, db);
        var booking = await DomainTestData.CreateBookingAsync(db, member, slot);
    ...
    public async Task T05_GetEligible_TwoEligibleOnePastLock_ReturnsTwoEligible()
    {
        ...
        var date = DateOnly.FromDateTime(DateTime.Today.AddDays(-2));
        var (_, slot1) = await DomainTestData.CreateSeasonAndSlotAsync(seasonService, db, date, new TimeOnly(8, 0));
        var (_, slot2) = await DomainTestData.CreateSeasonAndSlotAsync(seasonService, db,
            DateOnly.FromDateTime(DateTime.Today.AddDays(-3)), new TimeOnly(10, 0));

        // Third slot ~1 hour ago — inside the 2h single-player lock
        // CreateSlotAtAsync bypasses operating-hours constraints so this works at any time of day.
        var slotTime = DateTime.Now.AddHours(-1);
        var slot3 = await DomainTestData.CreateSlotAtAsync(db, slotTime);

        await DomainTestData.CreateBookingAsync(db, member, slot1);
        await DomainTestData.CreateBookingAsync(db, member, slot2);
        await DomainTestData.CreateBookingAsync(db, member, slot3);
    ...
    public async Task T06_GetEligible_TwoPlayerBookingPast2h30mLock_Included()
    {
        ...
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var member = await DomainTestData.CreateMemberAsync(userManager, db, level, "m1@t.com", "A", "B");
        var extra = await DomainTestData.CreateMemberAsync(userManager, db, level, "m2@t.com", "C", "D");
        var (_, slot) = await PastSlotAsync(seasonService, db);
        await DomainTestData.CreateBookingAsync(db, member, slot, [extra]);
    ...
    public async Task T07_GetEligible_ThreePlayerBookingPast3hLock_Included()
    {
        ...
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var member = await DomainTestData.CreateMemberAsync(userManager, db, level, "m1@t.com", "A", "B");
        var extra1 = await DomainTestData.CreateMemberAsync(userManager, db, level, "m2@t.com", "C", "D");
        var extra2 = await DomainTestData.CreateMemberAsync(userManager, db, level, "m3@t.com", "E", "F");
        var (_, slot) = await PastSlotAsync(seasonService, db);
        await DomainTestData.CreateBookingAsync(db, member, slot, [extra1, extra2]);
    ...
    public async Task T08_GetEligible_FourPlayerBookingPast3h30mLock_Included()
    {
        ...
        var extra1 = await DomainTestData.CreateMemberAsync(userManager, db, level, "m2@t.com", "C", "D");
        var extra2 = await DomainTestData.CreateMemberAsync(userManager, db, level, "m3@t.com", "E", "F");
        var extra3 = await DomainTestData.CreateMemberAsync(userManager, db, level, "m4@t.com", "G", "H");
        var (_, slot) = await PastSlotAsync(seasonService, db);
        await DomainTestData.CreateBookingAsync(db, member, slot, [extra1, extra2, extra3]);
    ...
    public async Task T09_GetEligible_MemberIsOnlyAdditionalParticipant_NotInEligibleList()
    {
        ...
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var primaryBooker = await DomainTestData.CreateMemberAsync(userManager, db, level, "booker@t.com", "A", "B");
        var secondaryMember = await DomainTestData.CreateMemberAsync(userManager, db, level, "secondary@t.com", "C", "D");
        var (_, slot) = await PastSlotAsync(seasonService, db);

        // secondaryMember is only an additional participant — not the primary booker
        await DomainTestData.CreateBookingAsync(db, primaryBooker, slot, [secondaryMember]);
    ...
    public async Task T10_GetEligible_TwoMembers_NoLeakage()
    {
        ...
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var member1 = await DomainTestData.CreateMemberAsync(userManager, db, level, "m1@t.com", "A", "B");
        var member2 = await DomainTestData.CreateMemberAsync(userManager, db, level, "m2@t.com", "C", "D");

        var (_, slot1) = await PastSlotAsync(seasonService, db);
        var (_, slot2) = await DomainTestData.CreateSeasonAndSlotAsync(seasonService, db,
            DateOnly.FromDateTime(DateTime.Today.AddDays(-3)), new TimeOnly(10, 0));

        await DomainTestData.CreateBookingAsync(db, member1, slot1);
        await DomainTestData.CreateBookingAsync(db, member2, slot2);
    ...
    public async Task T16_Submit_BookingOwnedByDifferentMember_ReturnsFalse()
    {
        ...
        var level = await DomainTestData.CreateMembershipLevelAsync(db, "SH", "Shareholder");
        var booker = await DomainTestData.CreateMemberAsync(userManager, db, level, "booker@t.com", "A", "B");
        var otherMember = await DomainTestData.CreateMemberAsync(userManager, db, level, "other@t.com", "C", "D");
        var (_, slot) = await PastSlotAsync(seasonService, db);
        var booking = await DomainTestData.CreateBookingAsync(db, booker, slot);
    ...
    public async Task T26_Submit_ConcurrentDuplicates_SecondFails()
    {
        ...
        int bookingId, memberId;
        {
            ...
            var seasonService = seedScope.ServiceProvider.GetRequiredService<SeasonService>();
        ...
        var svc1 = new ScoreService(
            scope1.ServiceProvider.GetRequiredService<AppDbContext>(),
        ...
        var svc2 = new ScoreService(
            scope2.ServiceProvider.GetRequiredService<AppDbContext>(),
    ...
    public async Task T27_Submit_SaveChangesThrows_RollsBackAndReturnsFalse()
    {
        ...
        var throwingDb = new ThrowingOnSaveDbContext(provider.GetRequiredService<DbContextOptions<AppDbContext>>());
    ...
    public async Task T29_GetRounds_TwoRounds_ReturnedOrderedBySubmittedAtDescending()
    {
        ...
        var (_, slot1) = await DomainTestData.CreateSeasonAndSlotAsync(seasonService, db,
        ...
        var booking1 = await DomainTestData.CreateBookingAsync(db, member, slot1);
        var booking2 = await DomainTestData.CreateBookingAsync(db, member, slot2);
    ...
    public async Task T30_GetRounds_TwoMembers_NoLeakage()
    {
        ...
        var booking1 = await DomainTestData.CreateBookingAsync(db, member1, slot1);
        var booking2 = await DomainTestData.CreateBookingAsync(db, member2, slot2);
